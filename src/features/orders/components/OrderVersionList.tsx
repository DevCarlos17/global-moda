...
export function OrderVersionList({ orderId }: OrderVersionListProps) {
  ...
  return (
    <div className="bg-surface border border-white/10 rounded-2xl p-6">
      <p className="text-xs text-white/40 uppercase tracking-wider mb-4">Historial de versiones</p>
        ...
        <ul className="flex flex-col gap-4">
          {versions.map((v) => (
            <li key={v.id} className="flex gap-4 items-start pl-2">
              {/* Dot */}
              <div className="size-2 rounded-full bg-gold mt-1.5 flex-shrink-0" />
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-sm font-medium text-white">{v.order_number}</span>
